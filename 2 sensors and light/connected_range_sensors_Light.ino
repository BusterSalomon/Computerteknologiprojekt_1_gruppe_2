...
int LightlvlLedState = LOW; // ledState for light level sensor.
...
// Read light level
int readlight(int input) {
  int lightLvl = analogRead(input);
  return lightLvl;
} 

void checkLightLvl(int input, int output, int* ledState) {
  int lightLvl = readlight(input);
  if (lightLvl < 15) {  // if the light level is below 15 the light will be turned off
    if (*ledState != LOW) {
...
void loop() {
  ...
  Serial.println(readlight(inputPinLight));
  
  checkLightLvl(inputPinLight, ledPin3, &LightlvlLedState); // checks ligh level and changes led state if ligh level is below 15 or above 14
