...
#                 linear_vel, collision_flag = self.get_linear_vel(center_avrg, angular_vel, direction)
...
class Obstacle():
    ...
    # Post: Returns 3 arrays with lidar scans, left, center and right
    def split_scan(self, lidar_distances):
        lidar_left = [d for d in lidar_distances[:30] if d != 0]
        lidar_center = [d for d in lidar_distances[26:66] if d != 0]
        lidar_right = [d for d in lidar_distances[61:] if d != 0]
        return (lidar_left, lidar_center, lidar_right)
    ...
    def get_direction (self, left_avrg, center_avrg, right_avrg):
        direction = 0
        if (center_avrg < TURNING_DISTANCE):
            if (abs(left_avrg - right_avrg) < 0.1 and center_avrg > SAFE_STOP_DISTANCE and (left_avrg + right_avrg) / 2 < 1.0): # need explanation
                direction = 0
                rospy.loginfo("going STRAIGHT!")
            elif(left_avrg > right_avrg ):
                direction = -1
    ...
    def get_angular_vel(self, center_avrg):
        if (center_avrg < TURNING_DISTANCE):
            # angular_vel = abs(ANGULAR_MAX_VEL - (center_avrg * 1.4)) * TURNING_CONSTANT # !!!SHOULD BE REFACTORED!!!
            angular_vel = 1.3602*center_avrg**2-4.74396*center_avrg+4.13376
        else: 
            angular_vel = 0
        return angular_vel
    ...
    def get_linear_vel(self, center_avrg, direction):
        ...
        if center_avrg < SAFE_STOP_DISTANCE:
            ...
            linear_vel = 0.107659 * center_avrg**2+0.383663*center_avrg-0.121094
            #linear_vel = (LINEAR_VEL - (angular_vel / (ANGULAR_MAX_VEL*5)) / 3)*direction
        return linear_vel, collision_flag
        ...
        while not rospy.is_shutdown():
            ...
            if (lidar_center and lidar_left and lidar_right): # only if the arrays are not empty
                ...
                if center_avrg < SAFE_STOP_DISTANCE and turtlebot_moving:
                    ...
                    twist.linear.x = 0.107659 * center_avrg**2+0.383663*center_avrg-0.121094
                    turtlebot_moving = True
